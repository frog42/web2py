...
sudo mv /var/www/web2py/* /var/www/
mv handlers/wsgihandler.py ./wsgihandler.py
chown -R testbed:testbed ./
...
echo "================================================="
echo '
WSGIDaemonProcess testbed user=testbed group=testbed processes=1 threads=1
<VirtualHost *:80>
RewriteEngine On
...
WSGIProcessGroup testbed
WSGIScriptAlias / /var/www/wsgihandler.py
...
/var/www/applications/$1/static/$2
<Directory /var/www/applications/*/static/>
Options -Indexes
ExpiresActive On
...
cd /var/www
sudo -u testbed python -c "from gluon.widget import console; console();"
sudo -u testbed python -c "from gluon.main import save_password; save_password(raw_input('admin password: '),443)"
echo "done!"
